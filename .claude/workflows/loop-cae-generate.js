...
// inlined to avoid args-passing ambiguity; brief agents read the abstract from heldout_cae.json by hid
const items = [
  {hid:1,journal:'CircAE'},{hid:2,journal:'CircAE'},{hid:3,journal:'CircAE'},{hid:4,journal:'CircAE'},
];
...
Read ${DIR}/heldout_cae.json and use ONLY the object whose hid == ${it.hid} (target journal: ${it.journal}); ignore all other entries. Take its "abstract" field as the source.
...
Write the brief as plain text to ${DIR}/loop_cae/brief_${it.hid}.json in this exact JSON shape:
...
2. The study brief at ${DIR}/loop_cae/brief_${it.hid}.json
Do NOT read any other file. You have ONLY the brief — you have not seen any original abstract.

Using the skill's guidance, write the best possible abstract for the target journal "${it.journal}" from the brief's facts. Write ONLY the finished abstract (with section labels if the skill says that journal uses them) to ${DIR}/loop_cae/A_${it.hid}.txt.
Return {hid: ${it.hid}, done: true}.`;
}

function genBPrompt(it) {
  return `Read the study brief at ${DIR}/loop_cae/brief_${it.hid}.json. Do NOT read any other file.
Write the best possible abstract for the target journal "${it.journal}" from the brief's facts, using your own judgment (no special style guide). Write ONLY the finished abstract to ${DIR}/loop_cae/B_${it.hid}.txt.
Return {hid: ${it.hid}, done: true}.`;
}
