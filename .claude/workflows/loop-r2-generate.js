...
// inlined to avoid args-passing ambiguity; brief agents read the abstract from heldout_r2.json by hid
const items = [
...
Read ${DIR}/heldout_r2.json and use ONLY the object whose hid == ${it.hid} (target journal: ${it.journal}); ignore all other entries. Take its "abstract" field as the source.
...
Write the brief as plain text to ${DIR}/loop_r2/brief_${it.hid}.json in this exact JSON shape:
...
2. The study brief at ${DIR}/loop_r2/brief_${it.hid}.json
Do NOT read any other file. You have ONLY the brief — you have not seen any original abstract.

Using the skill's guidance, write the best possible abstract for the target journal "${it.journal}" from the brief's facts. Write ONLY the finished abstract (with section labels if the skill says that journal uses them) to ${DIR}/loop_r2/A_${it.hid}.txt.
Return {hid: ${it.hid}, done: true}.`;
}

function genBPrompt(it) {
  return `Read the study brief at ${DIR}/loop_r2/brief_${it.hid}.json. Do NOT read any other file.
Write the best possible abstract for the target journal "${it.journal}" from the brief's facts, using your own judgment (no special style guide). Write ONLY the finished abstract to ${DIR}/loop_r2/B_${it.hid}.txt.
Return {hid: ${it.hid}, done: true}.`;
}
