export const meta={name:'loop-t2-generate',description:'Title round-3: regenerate A with improved (info-density) skill',phases:[{title:'Generate'}]}
...
function genA(hid){return `Read three files: the title skill ${SKILL}, its journal profiles ${PROF}, and the study brief ${D}/loop_r2/brief_${hid}.json (names the target journal). Do NOT read other files. Follow the skill — especially Step 2a (pack in the specific finding/outcomes, both comparison arms, named device/study, cohort size/design) before fitting style. Write the SINGLE best, most informative title that still fits the journal to ${D}/loop_t3/A_${hid}.txt (one line, no quotes). Return {hid:${hid},done:true}.`;}
phase('Generate');
const r=await parallel(items.map(h=>()=>agent(genA(h),{label:`A3:${h}`,phase:'Generate',schema:RET})));
log(`t3 A regenerated: ${r.filter(Boolean).length}/15`); return {ok:r.filter(Boolean).length};
